...
- **Both routes that render item detail need `generateStaticParams`, not just one.** `app/item/[id]/page.tsx` (the standalone deep-link page) got it first, fixing a real measured slowness (347-728ms down to 45-78ms). `app/@modal/(.)item/[id]/page.tsx` (the intercepted modal, what actually fires every time someone taps a card from the grid, i.e. the normal browsing flow) was missed in that pass and stayed `ƒ Dynamic`, meaning every single card tap did a live cross-region Supabase round trip. Caught 2026-08-01 after a real production report ("pressing an item takes so long"); confirmed via `next build` output (`ƒ` vs `●` markers) before guessing at a cause. If a third place ever renders item detail, check this list applies there too.
